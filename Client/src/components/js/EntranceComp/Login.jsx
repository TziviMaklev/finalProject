...
export default function Login() {
    ...
    async function onSubmitHandler(e) {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:3000/users/${inputs.username}/${inputs.password}`);
            ...
            if (response.ok) {
                console.log("ok1");
                if (data.length > 0) {
                    const response = await fetch(`http://localhost:3000/users/${data[0].user_id}`);
                    const details = await response.json();
                    console.log(details);
                    if (response.ok) {
                        localStorage.setItem("userInfo", JSON.stringify(details[0]));
                        setInputs({});
                        navigate(`/users/${data[0].user_id}/home`);
                    }
                    else {
            ...
            {keys.length == 0 &&
                <div>
                    <form onSubmit={onSubmitHandler}>
                        ...
                        <h5>Password:</h5>
                        <input className="elementInForm" name="password" value={inputs.password} onBlur={onBlurHandler}  type="text" placeholder="Password" required />
